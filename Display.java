...
public class Display {
    ...
    public void drawGrid(Thing[][] f, PImage playerPic) {
        Thing piece;
       // int picNum = frameNum % 36;
            ...
            for (int j = 0; j < numcols; j++) {
                ...
                if(piece instanceof  Obstacle){
                    ...
                    if (piece instanceof Player) {
                        pieceImage = playerPic;
                    } else {
                        //System.out.println(piece.imageNum);
                        pieceImage = getImage(piece.imageNum);
                    }
                        p.image(pieceImage, x + j * dx, y + i * dy, dx, dy);
                    p.fill(p.color(255,0,0));
                        p.text("Lvl:" + piece.level, x + j * dx, y + i * dy);
    ...
    public void highlightLocation(Location l, GameBoard g) {
        if (g.isOutGrid(l.getRow(),l.getCol())) {
    ...
    public void initializeWithGame(GameBoard graphic) {
        Thing[][] grid = graphic.getGrid();
        if (grid == null) {
            System.out
